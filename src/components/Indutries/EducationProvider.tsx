...
import { educationProviders } from "@/lib/data";
...
const EducationProvider = () => {
  return (
    <div className="bg-white pt-[64px] pb-[112px]">
      <Container className="space-y-4">
        <div className="space-y-1.5 md:space-y-2.5">
          <h1 className="text-[#213B3C] max-w-[535px] text-3xl md:text-[36px] font-bold">
            Comprehensive solutions for Education providers
          </h1>
          <div className="">
            {educationProviders.map((item, idx) => (
              <EducationCard key={item.id} {...item} index={idx + 1} />
            ))}
          </div>
        </div>
      </Container>
