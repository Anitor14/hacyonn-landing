...
import { financeProviders } from "@/lib/data";
      ...
      <Container className="space-y-4">
        <div className="space-y-1.5 md:space-y-2.5">
          <h1 className="text-[#213B3C] max-w-[535px] text-3xl md:text-[36px] font-bold">
            Comprehensive solutions for Fintech providers
          </h1>
          <div className="">
            {financeProviders.map((item, idx) => (
              <FinanceCard key={item.id} {...item} index={idx + 1} />
            ))}
          </div>
        </div>
      </Container>
